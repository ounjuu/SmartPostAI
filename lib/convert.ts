...
- 친근한 톤 (~해요, ~거든요, ~더라고요)
- 이모지 최소화, **문장 끝에 이모티콘 붙이지 말 것** (웃는 느낌은 ㅎㅎ, 우는/아쉬운 느낌은 ㅜㅜ로 표현)
...
- **소제목/섹션 헤더는 대괄호 태그 형식으로**: 이모지(📍💬🍝💡⚠️✅👉📌✨) X, 대신 \`[매장 정보]\`, \`[메뉴 후기]\`, \`[방문 후기]\` 형식 사용. 원본에 이모지 + 소제목 형식이 있어도 변환 시 대괄호 형식으로 바꾸기
- **별점(⭐ ★) 평가 금지**: 원본에 "맛 ⭐⭐⭐⭐⭐" 같은 별점 평가가 있어도 별점을 빼고 문장으로 풀어서 변환. "솔직 후기"라는 표현·소제목도 [방문 후기] 등으로 바꾸기
...
반드시 아래 JSON 형식으로만 응답해주세요 (다른 텍스트 없이):
{
  ...
  return {
    title: normalizeEmoticons(stripEmphasisQuotes(parsed.title, false)),
    content: normalizeEmoticons(stripEmphasisQuotes(parsed.content, true)),
    tistoryTitle: normalizeEmoticons(
      stripEmphasisQuotes(stripMarkdown(parsed.tistoryTitle || parsed.title), false)
    ),
    tistoryContent: normalizeEmoticons(
      stripEmphasisQuotes(stripMarkdown(parsed.tistoryContent || ""), false)
    ),
